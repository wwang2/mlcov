...
def compute_phase(N, m, p):
    ...
    pickle.dump(sim_data, open( 'data/small_world_N_{}m{}_p{}_mesh{}.pkl'.format(N, m, p, res_r * res_t), "wb" ) )
    ...
    plt.colorbar()
    plt.savefig('figure/small_world_mean_N_{}m{}_p{}_mesh{}.jpg'.format(N, m, p, res_r * res_t))
    ...
    plt.plot(np.linspace(0.05, 0.8), np.linspace(0.05, 0.4))
    plt.colorbar()
    plt.savefig('figure/small_world_var_N_{}m{}_p{}_mesh{}.jpg'.format(N, m, p, res_t * res_r))



for p in [0.025, 0.05, 0.2, 0.5]:

    compute_phase(5000, 4, p)